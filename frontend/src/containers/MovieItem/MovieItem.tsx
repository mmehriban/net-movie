...
const MovieItem = forwardRef<HTMLDivElement, IMovieItemProps>((props, ref) => {
  ...
  const handleExpandClick = (movieId: string) => {
    ...
    openMovieDetailOf(movieId); // Pass the movie ID to the callback function
    window.scrollTo(0, 0);
    console.log(movieId);
  };
    ...
    <div className='flex ml-16 text-white translate-x-0 duration-500' ref={ref}>
      {item.movie_list.map((movie:any) => (
        <div key={movie.id} className='' >
          <div  className='group rounded-md relative h-32' 
                ...
                '>
                    ...
                    <div className='' onClick={() =>{}}> 
                      <div className='flex justify-between items-center'>
                          ...
                          <div >
                          <ExpandCircleDownRoundedIcon  onClick={(): any => handleExpandClick(movie.id)}  className=' hover:opacity-70 transition duration-200 cursor-pointer pr-1 text-stone-400'
                          style={{width: '35px', height: '35px'}}/>
                          </div>
                      </div>
                      
                      <p style={{fontSize: '10px'}} className='text-xs font-semibold text-green-400 pt-1'>Best of <span className='text-white'>{movie.release_date.slice(0,4)}</span>
